...
public class CalendarioDisponibilidadInstalacion extends JDialog {
	 ...
	 * Create the frame.
	 */
	public CalendarioDisponibilidadInstalacion(Programa p) {
		this.p = p;
		llenarMeses();
		llenarAños();
		...
		iniciarMes("Diciembre", 2020);
	}
	
//	public CalendarioDisponibilidadInstalacion(Programa p, String mes, int año) {
//		this(p);
//		iniciarMes(mes,año);
//	}
	...
	private Component paintDia(int numeroDia) {
		...
		if (numeroDia != -1) {
			...
			int mes = parseMonthString().get((String)cbMeses.getSelectedItem());
			int año = (Integer)cbAños.getSelectedItem();
			try {
				if(p.instalacionDisponibleDia(i, numeroDia, mes, año)) {
	...
	private HashMap<String, Integer> parseMonthString() {
		HashMap<String, Integer> parser = new HashMap<>();
		parser.put("Enero", 1);
		parser.put("Febrero", 2);
		parser.put("Marzo", 3);
		parser.put("Abril", 4);
		parser.put("Mayo", 5);
		parser.put("Junio", 6);
		parser.put("Julio", 7);
		parser.put("Agosto", 8);
		parser.put("Septiembre", 9);
		parser.put("Octubre", 10);
		parser.put("Noviembre", 11);
		parser.put("Diciembre", 12);
		...
		if (cbInstalacion == null) {
			...
			cbInstalacion.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					iniciarMes((String)cbMeses.getSelectedItem(), (Integer)cbAños.getSelectedItem());
				}
			});
